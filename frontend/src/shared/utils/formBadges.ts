...
export function badgeFromMeanRating(meanRating: number | null): FormBadge {
  ...
  if (meanRating >= 50) return { label: "폼(평점) ↑", color: "teal", hint: "최근 평점이 좋음", valueText: meanRating.toFixed(1) };
...
export function badgeFromWinRate(winRate: number | null): FormBadge {
  ...
  if (winRate >= 0.75) return { label: "승률 🔥", color: "teal", hint: "최근 승률이 높음", valueText: `${pct}%` };
  if (winRate >= 0.70) return { label: "승률 ↑", color: "lime", hint: "최근 승률이 좋은 편", valueText: `${pct}%` };
  if (winRate >= 0.66) return { label: "승률 →", color: "gray", hint: "평균적", valueText: `${pct}%` };
