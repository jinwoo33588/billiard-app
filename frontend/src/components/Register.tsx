...
import { TextInput, PasswordInput, Button, Card, Title, Stack, Text, Anchor, NumberInput } from '@mantine/core';
...
function Register({ onToggleForm }: RegisterProps) {
  const form = useForm({
    ...
    validate: {
      ...
      handicap: (value) => (value > 0 ? null : '점수를 입력하세요.'),
    },
  });
    ...
    <Card shadow="sm" p={{ base: 'md', sm: 'lg' }} radius="md" withBorder w={400}>
      <Stack>
        <form onSubmit={form.onSubmit(handleSubmit)}>
          ...
          <NumberInput mt="md" label="핸디캡 점수" placeholder="본인의 점수를 입력하세요" required {...form.getInputProps('handicap')} />
