// backend/services/insights/insights.scoring.js
const { clamp, round, num } = require("./insights.utils");

/**
 * GPS Scoring (Absolute, per-game)
 * - effScore: eff(avg - expected)를 "중앙 둔감 / 꼬리 민감"으로 0~100 변환
 * - volScore: vol((score - handicap)/sqrt(inning))을 "중앙 둔감 / 꼬리 민감"으로 0~100 변환
 * - gps: effScore / volScore 중 더 잘 나온 쪽에 softmax로 가중치
 */
...
function gpsScore(eff, vol) {
  const es = effScore(eff); // 0~100
  const vs = volScore(vol); // 0~100
...
function scoreGame({ score, inning, expected, handicap }) {
  ...
  return {
    ...
    // 디버깅/프론트 표시용(원하면 explain 패널에 노출)
    scoringParams: {
      R_EFF,
      P_EFF,
      R_VOL,
